...
// concurrent logins into one event-loop stall - and here that loop is also the
// poll loop), and the stored hash format must not have changed with it - a
// hash minted by the old synchronous code still has to verify. Plain node +
// assert, throwaway data dir.
...
process.env.SNMPCANVAS_DATA = fs.mkdtempSync(path.join(os.tmpdir(), 'snmpc-auth-'));
